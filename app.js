...
const io =ws(app);
let i =0
io.on('connection',function(socket){    //第二次握手
    console.log('有用户进入');
    i++;
    console.log(i); 
    //服务端的第三次握手服务器通过onmessage事件进行数据交互
    socket.on('message',async function(mes){
        console.log(mes);        
       await io.emit('message',mes);//发消息事件
    })    
});
app.listen('3000');
console.log('服务器连接成功','http://192.168.73.184:3000');
